...
class AdminsController
{
	...
	public function resetPasswordTestable($email, $requestMock, $templateMock){
		// 1. Consultar si existe el admin
		$url = "admins?linkTo=email_admin&equalTo=" . $email . "&select=id_admin";
		$admin = $requestMock("GET", $url, []);
		...
		$newPassword = $templateMock->genPassword(11);
		$crypt = crypt($newPassword, '$2a$07$azybxcags23425sdg23sdfhsd$');

		// 3. Actualizar contraseña
		$urlUpdate = "admins?id=" . $admin->results[0]->id_admin . "&nameId=id_admin&token=no&except=password_admin";
		$update = $requestMock("PUT", $urlUpdate, "password_admin=" . $crypt);

		if ($update->status != 200) {
			return ["status" => "error", "message" => "no-actualizado"];
		}

		// 4. Enviar correo
		$send = $templateMock->sendEmail(
			"Solicitud de nueva contraseña",
			$email,
			'SOLICITUD DE NUEVA CONTRASEÑA',
			"Su nueva contraseña: " . $newPassword,
			"http://localhost"
		);
		...
		return ["status" => "success", "newPassword" => $newPassword];
	}
}
